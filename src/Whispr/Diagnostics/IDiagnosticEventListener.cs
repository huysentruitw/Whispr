...
internal interface IDiagnosticEventListener
{
    ...
    IDisposable Start(string busName);
    ...
    /// <returns>An <see cref="IDisposable"/> that can be used to stop listening to the event.</returns>
    IDisposable Publish<TMessage>(string busName, Envelope<TMessage> envelope)
    ...
    /// <returns>An <see cref="IDisposable"/> that can be used to stop listening to the event.</returns>
    IDisposable Send(string busName, string topicName, SerializedEnvelope envelope);
    ...
    /// <returns>An <see cref="IDisposable"/> that can be used to stop listening to the event.</returns>
    IDisposable Consume(string busName, string consumerName, string queueName, SerializedEnvelope envelope);
}
